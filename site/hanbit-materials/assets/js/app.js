(() => {
  ...
  const state = { lang: chooseLanguage(), material: chooseMaterial(), inquiry:{type:'material',material:'',formGrade:'',dimensions:'',quantity:'',application:'',temperature:'',pressure:'',media:'',documents:'',partType:'',delivery:'',company:'',name:'',email:'',drawing:'',critical:''} };
  ...
  function renderInquiry() {
    const q=state.inquiry; const machiningFields=q.type==='machined'?`<label class="field">${label('partType')}<select name="partType"><option value="">${t('notSure')}</option>${machined.categories.map(item=>`<option value="${item.id}" ${q.partType===item.id?'selected':''}>${item.copy[state.lang].title}</option>`).join('')}</select></label><label class="field">${label('drawingAvailability')}<select name="drawing"><option value="">${t('notSure')}</option><option value="yes" ${q.drawing==='yes'?'selected':''}>${t('drawingAttached')}</option><option value="later" ${q.drawing==='later'?'selected':''}>${t('drawingLater')}</option></select></label><label class="field wide">${label('criticalRequirement')}<input name="critical" value="${escapeHTML(q.critical)}"></label>`:'';
    ...
    document.querySelector('#inquiryBuilder').innerHTML=`<form class="inquiry-form" id="inquiryForm"><div class="form-grid">
      ...
      <label class="field">${label('formGrade')}<input name="formGrade" value="${escapeHTML(q.formGrade)}"></label><label class="field">${label('dimensions')}<input name="dimensions" value="${escapeHTML(q.dimensions)}"></label><label class="field">${label('quantity')}<input name="quantity" value="${escapeHTML(q.quantity)}"></label>
      <label class="field wide">${label('applicationRequirement')}<textarea name="application">${escapeHTML(q.application)}</textarea></label>${machiningFields}<label class="field">${label('temperature')}<input name="temperature" value="${escapeHTML(q.temperature)}"></label><label class="field">${label('pressure')}<input name="pressure" value="${escapeHTML(q.pressure)}"></label><label class="field wide">${label('media')}<input name="media" value="${escapeHTML(q.media)}"></label><label class="field wide">${label('documents')}<input name="documents" value="${escapeHTML(q.documents)}"></label><label class="field wide">${label('delivery')}<input name="delivery" value="${escapeHTML(q.delivery)}"></label>
  ...
  function buildSummary() {
    ...
    const labels=[['inquiryType',inquiryTypeLabel()],['interestMaterial',material],['partType',q.partType],['formGrade',q.formGrade],['dimensions',q.dimensions],['quantity',q.quantity],['applicationRequirement',q.application],['temperature',q.temperature],['pressure',q.pressure],['media',q.media],['documents',q.documents]]; if(q.type==='machined')labels.push(['drawingAvailability',q.drawing],['criticalRequirement',q.critical]); labels.push(['delivery',q.delivery],['companyName',q.company],['contactName',q.name],['replyEmail',q.email]);
