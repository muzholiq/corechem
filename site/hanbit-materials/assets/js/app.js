(() => {
  ...
  function renderSolutions() {
    document.querySelector('#solutionGrid').innerHTML=flow.solutions.map(solution=>{const copy=solution.copy[state.lang];return `<article class="solution-card"><span class="solution-code">${solution.code}</span><h3>${copy.title}</h3><p>${copy.text}</p><div class="solution-materials" aria-label="${t('reviewCandidates')}">${solution.materials.map(id=>`<span>${materials.find(item=>item.id===id).name}</span>`).join('')}</div><button type="button" data-solution-material="${solution.materials[0]}">${t('openMaterial')} →</button></article>`}).join('');
